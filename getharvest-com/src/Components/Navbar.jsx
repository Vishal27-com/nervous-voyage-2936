...
const Navbar = () => {
    return (
        <Box h='80px' boxShadow='base' position='sticky' top='0px' bgColor={['black','','#FBF6EF']}>
         ...
         <Box color={['white','','#FA5D00']} borderRight='1px solid lightgrey' pr='10px'>
          ...
          <Box >
            <HStack spacing='15px'>
                {
                    links.map(el=>
                        <Link to={el.to} key={el.title}><Text fontSize='18px'>{el.title}</Text></Link>
                        )
                    }
            </HStack>
          </Box>
            ...
            <Box display={['block','block','none']}><MyAccord links={links} /></Box>
        </Box>
    );
};
